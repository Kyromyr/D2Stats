...
Func GetRemoteProcAddress($Proc, $sModule, $lpProc)
	Switch VarGetType($lpProc)
		Case "Int32" ;Ordinal passed
			Local $Type = "dword"
			Local $Value = _WinAPI_LoWord($lpProc < 0 ? -$lpProc : $lpProc)
...
Func _Assemble($hProc, $sParameter, $Type, $Register = 0)
	Local $sType = StringInStr($Type, "static") ? StringTrimLeft($Type, 6) : $Type
	Select
...
Func __ProcCall_Startup()
	...
	If Not $__ProcCall_Start Then
		$__ProcCall_Start += 1
		If Not IsDeclared("__Dll__HandleIndex") Then Global $__Dll__HandleIndex[4] = [0]
		$__Dll__HandleIndex[1] = DllOpen("Kernel32.dll")
		$__Dll__HandleIndex[2] = DllOpen("Advapi32.dll")
		$__Dll__HandleIndex[3] = DllOpen("User32.dll")
